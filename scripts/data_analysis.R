...
t3_30_300_rank_long_gdf <- t3_30_300_rank_gdf |> 
    ...
    mutate(Metric = fct_relevel(Metric, c('IMDScore_rank', 'IncScore_rank', 'EmpScore_rank',
                                          'EduScore_rank', 'HDDScore_rank', 'CriScore_rank',
                                          'BHSScore_rank', 'EnvScore_rank', 'area_rank',
                                          'Total_rank', 'tree_count_rank', 'park_distance_rank',
                                          'canopy_cover_rank', 'water_distance_rank',
                                          'NDVI_rank', 'NDBI_rank', 'NDWI_rank')))
...
t3_30_300_spectral_rank_map <- t3_30_300_rank_rgn_gdf |> 
    ...
    geom_sigmoid(aes(x = X, y = Y, xend = x_start + x_span * 0, yend = tree_count_rank, group = RGN22CD, color = IMD)) +
