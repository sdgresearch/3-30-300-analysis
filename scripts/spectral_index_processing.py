...
# Function to calculate the median for each feature (out of the max values for a given date range)
def calculate_median_index(image_collection: ee.image.Image, geometries: ee.featurecollection.FeatureCollection, 
    ...
    median_dict = image_collection.reduceRegions(
...
def calculate_zonal_statistics(imd_lsoa_bua_boundaries_ee: ee.featurecollection.FeatureCollection, iter_span: int=100) -> list:
    ...
    end_time = time.time()
    logging.warning(f"Calculating spectral zonal statistics took {end_time - start_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    spectral_indexes_path = T3_30_300_DIR / f"spectral_indexes_{start_date}_{end_date}.geojson"

    log_path = Path("logs/spectral_indexes_calculation.log")
    setup_logger(log_path=log_path, log_level=log_level)
    logging.warning(f"Calculating spectral indexes for all geographies between {start_date} and {end_date}")

    logging.warning("Initializing GEE")
    ee.Authenticate()
    ee.Initialize(project=GEE_PROJECT_NAME, opt_url='https://earthengine-highvolume.googleapis.com')

    logging.warning("Querying GEE for spectral indexes")
