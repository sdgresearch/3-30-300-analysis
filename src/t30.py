...
def generate_tiles_paths(geo_level: str, geo_code: str, output_areas_os_tile_overlay_df: gpd.GeoDataFrame, vom_raster_paths_df) -> tuple:

    logging.debug(f"Generating tile paths for {geo_code}")
    ...
    return geo_vom_tiles_df, geo_selected_vom_tiles_df

def select_chm_files(geo_vom_tiles_df: pd.DataFrame, geo_selected_vom_tiles_df: pd.DataFrame) -> list:
    """
    ...
    for row in geo_selected_vom_tiles_df.itertuples():
        ...
        year = row.year
        temp_df = vom_raster_paths_df[((vom_raster_paths_df['TILE_NAME'] == tile_name) & (vom_raster_paths_df['year'] == year)) & (vom_raster_paths_df['file_type'] == 'CHM')]
        if len(temp_df) == 1:
            ...
            while len(tile_years_lst) > 0 and ind < len(tile_years_lst):
                temp_df = vom_raster_paths_df[((vom_raster_paths_df['TILE_NAME'] == tile_name) & (vom_raster_paths_df['year'] == tile_years_lst[ind])) & (vom_raster_paths_df['file_type'] == 'CHM')]
                if len(temp_df) == 1:
                    selected_chm_path_lst.append(Path(temp_df.path.iloc[0]))
...
def process_geo_code(geo_level: str, geo_code: str, output_areas_boundaries_gdf: gpd.GeoDataFrame, 
                     output_areas_os_tile_overlay_df, vom_raster_paths_df,
    ...
    try:
        ...
        geo_vom_tiles_df, geo_selected_vom_tiles_df = generate_tiles_paths(geo_level, geo_code, output_areas_os_tile_overlay_df, vom_raster_paths_df)
        selected_chm_path_lst = select_chm_files(geo_vom_tiles_df, geo_selected_vom_tiles_df)
